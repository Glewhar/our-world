...
type RailKind = 'nuke' | 'warming' | 'ice' | 'asteroid' | 'virus' | 'volcano' | 'tsunami' | 'ai';
type RailButton = {
  kind: RailKind;
  icon: string;
  label: string;
...
export type DisasterRailMount = {
  /** Per-frame poll (climate slot disabled state). */
  update(): void;
  dispose(): void;
};

export function mountDisasterRail(
  host: HTMLElement,
  ...
  let armed: { kind: RailKind; btn: HTMLButtonElement; wrap: HTMLElement; readValue: () => number } | null = null;

  const onDocPointerDown = (ev: Event): void => {
    if (!armed) return;
    const t = ev.target as Node | null;
    if (!t) return;
    if (armed.wrap.contains(t)) return;
    if (armed.btn.contains(t)) return; // btn click handler will run
    disarm();
  };
  ...
  function arm(kind: RailKind, btn: HTMLButtonElement, wrap: HTMLElement): void {
    if (armed?.kind === kind) return;
    disarm();
    let panel: PanelBuild;
    if (kind === 'nuke') panel = buildNukePanel();
    else if (kind === 'warming') panel = buildHeatPanel();
    else panel = buildIcePanel();
  ...
  function fire(kind: RailKind, value: number): void {
    if (kind === 'nuke') {
      const cfg = DEFAULT_NUCLEAR_WAR_CONFIG;
      const payload: NuclearWarScenarioPayload = {
        schedule: [],
        strikeCount: value,
    ...
    if (kind === 'warming') {
      if (registry.climateSlotsFull()) return;
      const payload: GlobalWarmingScenarioPayload = { maxTempDeltaC: value };
      registry.start('globalWarming', payload, totalDaysProvider(), DEFAULT_GLOBAL_WARMING_CONFIG.durationDays, { label: 'Heatwave' });
      return;
    }
    if (kind === 'ice') {
      if (registry.climateSlotsFull()) return;
      const payload: IceAgeScenarioPayload = { maxTempDeltaC: value };
  ...
  const climateButtons: HTMLButtonElement[] = [];
  const elsByKind = new Map<RailKind, HTMLButtonElement>();
  ...
  for (let i = 0; i < buttons.length; i++) {
    ...
    if (i === 3) {
      ...
      if (!buttons.slice(3).some(x => x.active)) div.style.display = 'none';
    ...
    if (!b.active) {
      ...
      btn.addEventListener('click', () => {
        if (b.isClimate && registry.climateSlotsFull()) return;
      ...
      if (b.isClimate) climateButtons.push(btn);
    }
  ...
  function update(): void {
    ...
    for (const btn of climateButtons) {
      ...
      if (want !== has) {
        ...
        if (want && armed?.kind === btn.dataset['kind']) disarm();
      }
    }
  }
...
function buildNukePanel(): PanelBuild {
...
function buildHeatPanel(): PanelBuild {
...
function buildIcePanel(): PanelBuild {
  return buildThermometerPanel({
    kind: 'ice',
    min: ICE_MIN,
    max: ICE_MAX,
...
function buildThermometerPanel(opts: {
  ...
  const fillFor = (v: number): number => {
    if (opts.kind === 'heat') return (v - opts.min) / (opts.max - opts.min);
