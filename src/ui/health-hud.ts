...
import type { BiomeCategoryShares } from '../world/scenarios/healthSnapshot.js';
import {
  ICON_BIOSPHERE,
  ICON_CAT_DESERT,
  ICON_CAT_GRASSLAND,
  ICON_CAT_RAINFOREST,
  ICON_CAT_TEMPERATE_FOREST,
  ICON_CAT_TUNDRA_ICE,
  ICON_CAT_WASTELAND,
  ICON_CITY_PIP,
  ICON_CITY_RUIN,
...
export type HealthHudMount = {
  /** Re-read registry snapshot and refresh every module. Safe each frame. */
  update(): void;
  /** Tear down DOM nodes. */
  dispose(): void;
};
...
const CAT_ORDER: readonly BiomeCategory[] = [
  'rainforest',
  'temperateForest',
  'grassland',
  'desert',
...
const CAT_GLYPH: Record<BiomeCategory, string> = {
...
export function mountHealthHud(
  host: HTMLElement,
...
/* -------------------------------------------------------------------------
 * Module 1 — Biome hex diorama (12 hexes, largest-remainder allocation).
 * ------------------------------------------------------------------------- */

type BiomeModule = {
  update(shares: BiomeCategoryShares): void;
  dispose(): void;
};

function mountBiomeHexes(host: HTMLElement): BiomeModule {
  const root = makeModuleShell(host, 'biome', 'Biome', ICON_BIOSPHERE);
  const row = document.createElement('div');
  row.className = 'biome-hex-row';
  const hexes: HTMLDivElement[] = [];
  const current: (BiomeCategory | null)[] = [];
  ...
  function update(shares: BiomeCategoryShares): void {
 ...
 */
function allocateHexes(shares: BiomeCategoryShares): BiomeCategory[] {
  let total = 0;
  for (const cat of CAT_ORDER) total += shares[cat];
  if (total <= 0) return [];
  const exact: Array<{ cat: BiomeCategory; floor: number; rem: number }> = [];
  let assigned = 0;
  ...
  const out: BiomeCategory[] = [];
  for (const e of exact) {
    for (let k = 0; k < e.floor; k++) out.push(e.cat);
  }
  return out;
}

/* -------------------------------------------------------------------------
 * Module 2 — Civilisation pips + alive/dead population band.
 * ------------------------------------------------------------------------- */

type CivModule = {
  update(citiesLost: number, citiesTotal: number, popLostPct: number): void;
  dispose(): void;
};

type CivPipState = 'intact' | 'ruin' | 'skull';

function mountCivPips(host: HTMLElement): CivModule {
  const root = makeModuleShell(host, 'civ', 'Civilization', ICON_CIV);
  const pipRow = document.createElement('div');
  pipRow.className = 'civ-pip-row';
  const pips: HTMLDivElement[] = [];
  const pipState: CivPipState[] = [];
  ...
  function update(citiesLost: number, citiesTotal: number, popLostPct: number): void {
    const ratio =
      citiesTotal > 0 ? Math.min(1, citiesLost / citiesTotal) : Math.min(1, popLostPct);
    const ruinCount = Math.round(ratio * CIV_PIPS);
    const allRuined = ratio >= 0.999;
    for (let i = 0; i < CIV_PIPS; i++) {
      let next: CivPipState;
      if (allRuined) next = 'skull';
      else if (i < ruinCount) next = 'ruin';
      else next = 'intact';
      if (next === pipState[i]) continue;
      const pip = pips[i]!;
...
/* -------------------------------------------------------------------------
 * Module 3 — Radiation strata bar + trefoil pip row (INVERTED fill).
 * ------------------------------------------------------------------------- */

type RadModule = {
  update(radiation01: number, bombsActive: number): void;
  dispose(): void;
};

function mountRadStrata(host: HTMLElement): RadModule {
  ...
  function update(radiation01: number, bombsActive: number): void {
...
/* -------------------------------------------------------------------------
 * Shared module shell — icon column + label row + visualization area.
 * Three rows: icon spans both content rows, label sits on row 1, viz
 * on row 2. No text sub-row, no numeric readout.
 * ------------------------------------------------------------------------- */
