...
export type ScenariosLauncherMount = {
  dispose(): void;
};
...
export function mountScenariosLauncher(
  _toggleEl: HTMLElement,
  panelEl: HTMLElement,
  registry: ScenarioRegistry,
  totalDaysProvider: () => number,
): ScenariosLauncherMount {
  ...
  const climateLaunchButtons: HTMLButtonElement[] = [];
  ...
  nukeLaunch.addEventListener('click', () => {
    ...
    const r = registry.start(
      'nuclear',
      payload,
      totalDaysProvider(),
      nukeSliders.durationDays.get(),
      { label: `Nuclear strike — ${region}` },
    );
    handleStartResult(r);
  });
  ...
  const warmingSliders = buildSliders(warming.body, [
    {
      key: 'maxTempDeltaC' as const,
      label: 'Max ΔT (°C)',
      min: 0, max: 30, step: 0.5, default: 8,
      formatValue: (v) => `+${v.toFixed(1)} °C`,
    },
    {
      key: 'maxSeaLevelM' as const,
  ...
  warmingLaunch.addEventListener('click', () => {
    ...
    const payload: GlobalWarmingScenarioPayload = {
    ...
    const r = registry.start(
      ...
      payload,
      totalDaysProvider(),
      warmingSliders.durationDays.get(),
  ...
  iceLaunch.addEventListener('click', () => {
    const payload: IceAgeScenarioPayload = {
  ...
  const warSliders = buildSliders(war.body, [
    {
      key: 'strikeCount' as const,
      label: 'Strike count',
      min: 10, max: 150, step: 1, default: warCfg.strikeCount,
      formatValue: (v) => `${v.toFixed(0)}`,
    },
    {
      key: 'strikeWindowDays' as const,
...
function handleStartResult(r: StartResult): void {
  if (r.ok) return;
...
function buildRow(host: HTMLElement, icon: string, title: string): RowEls {
...
function buildField(labelText: string): FieldEls {
...
function buildSliders<K extends string>(
  host: HTMLElement,
  specs: readonly SliderSpec<K>[],
): Record<K, SliderHandle> {
  const out = {} as Record<K, SliderHandle>;
...
function buildLaunchButton(label: string): HTMLButtonElement {
