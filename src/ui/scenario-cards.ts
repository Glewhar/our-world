...
export type ScenarioCardsMount = {
  /** Reconcile cards against the registry's active list. Cheap; safe each frame. */
  update(): void;
  /** Tear down all DOM nodes. */
  dispose(): void;
};
...
function progressOf(scn: Scenario): number {
...
function createCard(scn: Scenario, onStop: () => void): CardEls {
...
function refreshCard(card: CardEls, scn: Scenario, progress01: number): void {
  ...
  if (scn.kind === 'nuclear') {
    const p = scn.payload as { latDeg: number; lonDeg: number; radiusKm: number; stretchKm: number; windBearingDeg: number };
...
function formatCoords(latDeg: number, lonDeg: number): string {
