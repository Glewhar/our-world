...
import type {
  BiomeCategoryShares,
  WorldHealthSnapshot,
} from '../world/scenarios/healthSnapshot.js';
import type { BiomeCategory } from '../world/biomes/BiomeLookup.js';
import {
  ChevronDownIcon,
  ChevronUpIcon,
  CityBurningIcon,
...
type DesignKey = 'forest' | 'jungle' | 'grass' | 'desert' | 'tundra' | 'wasteland' | 'ocean' | 'mutated';

type DesignShares = Record<DesignKey, number>;
...
const HEX_COUNT = 12;
const DESIGN_ORDER: readonly DesignKey[] = [
  'forest', 'jungle', 'grass', 'desert', 'tundra', 'wasteland', 'mutated', 'ocean',
];

const DESIGN_GLYPH: Record<DesignKey, string> = {
  forest: TreeIcon,
  jungle: PalmIcon,
  grass: GrassIcon,
  desert: DesertIcon,
...
const SIM_TO_DESIGN: Partial<Record<BiomeCategory, DesignKey>> = {
...
type BiomeCard = {
  update(h: WorldHealthSnapshot, seaLevelM: number, tempC: number, landFraction01: number): void;
};

function buildBiomeCard(host: HTMLElement): BiomeCard {
  ...
  function update(h: WorldHealthSnapshot, seaLevelM: number, tempC: number, landFraction01: number): void {
...
function buildVitalsTile(host: HTMLElement, icon: string, label: string, tone: string): VitalsTile {
...
function formatSigned(v: number, unit: string, digits: number): string {
  if (Math.abs(v) < Math.pow(10, -digits - 1)) return `0${unit}`;
  const sign = v >= 0 ? '+' : '';
  return `${sign}${v.toFixed(digits)}${unit}`;
}

function simSharesToDesign(s: BiomeCategoryShares): DesignShares {
  const out: DesignShares = {
    forest: 0, jungle: 0, grass: 0, desert: 0,
    tundra: 0, wasteland: 0, ocean: 0, mutated: 0,
  };
  let landSum = 0;
  for (const k of Object.keys(s) as (keyof BiomeCategoryShares)[]) {
    const v = s[k];
    landSum += v;
    const designKey = SIM_TO_DESIGN[k];
    if (designKey) out[designKey] += v;
  }
...
type Tier = 'healthy' | 'middle' | 'end';

function tierFor(civ01: number): Tier {
  if (civ01 > 0.5) return 'healthy';
  if (civ01 > 0.1) return 'middle';
  return 'end';
}

function buildStat(host: HTMLElement, icon: string, label: string, kind: string): StatEls {
  const root = document.createElement('div');
...
function setStatTier(stat: StatEls, tier: Tier): void {
  if (stat.root.dataset['tier'] !== tier) stat.root.dataset['tier'] = tier;
}

function peopleIconFor(tier: Tier): string {
  if (tier === 'healthy') return PeopleNormalIcon;
  if (tier === 'middle') return PeopleDyingIcon;
  return PeopleDeadIcon;
}

function cityIconFor(tier: Tier): string {
  if (tier === 'healthy') return CityNormalIcon;
  if (tier === 'middle') return CityBurningIcon;
  return CityCollapsedIcon;
}

function roadIconFor(tier: Tier): string {
  if (tier === 'healthy') return RoadIcon;
  if (tier === 'middle') return RoadCrackingIcon;
...
type RadCard = {
  update(h: WorldHealthSnapshot): void;
};

function buildRadCard(host: HTMLElement): RadCard {
  const card = document.createElement('div');
  card.className = 'sp-card';
  card.id = 'rad-card';
  ...
  function update(h: WorldHealthSnapshot): void {
    ...
    let tier: 'low' | 'mod' | 'hi' | 'crit';
    let labelText: string;
    if (pct < 15)      { tier = 'low'; labelText = 'LOW'; }
    else if (pct < 50) { tier = 'mod'; labelText = 'MOD'; }
    else if (pct < 80) { tier = 'hi';  labelText = 'HI'; }
    else               { tier = 'crit'; labelText = 'CRIT'; }
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (pct >= 80) d.classList.add('crit');
      else if (pct >= 50) d.classList.add('hi');
