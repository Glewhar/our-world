 ...
 */
export function mountSettingsSheet(host, state, applyRenderScale, autoRenderScale, autoTierLabel, onPaneRefresh) {
    ...
    const existingTweakpane = host.querySelector('#tweakpane-host');
    if (existingTweakpane)
        existingTweakpane.remove();
    ...
    function toggle() { host.hidden = !host.hidden; }
    function hide() { host.hidden = true; }
    function dispose() { host.replaceChildren(); }
    return { toggle, hide, tweakpaneHost, dispose };
}
function pct(s) { return `${Math.round(s * 100)}%`; }
function buildToggle(host, label, initial, onChange) {
