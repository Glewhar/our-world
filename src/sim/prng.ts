...
export class Pcg32 {
  ...
  constructor(seed: bigint, stream = 0n) {
  ...
  serialize(): { state: string; inc: string } {
    return { state: this.state.toString(16), inc: this.inc.toString(16) };
  }

  /** Restore state from a serialized form. */
  static deserialize(snapshot: { state: string; inc: string }): Pcg32 {
    const r = Object.create(Pcg32.prototype) as Pcg32;
    Object.defineProperty(r, 'inc', {
      value: BigInt('0x' + snapshot.inc) & MASK_64,
      writable: false,
      enumerable: true,
    });
    (r as unknown as { state: bigint }).state = BigInt('0x' + snapshot.state) & MASK_64;
    return r;
  }
}

/**
 * Per-system PRNG identifiers. C6 mandates separate streams so tweaking one
 * system (e.g. balancing fire-spread) doesn't reshuffle every other system's
 * outputs.
 */
export type PrngStream = 'field' | 'entity_ai' | 'event_spawner' | 'role_mutation';

const STREAM_INDEX: Readonly<Record<PrngStream, bigint>> = Object.freeze({
  field: 0n,
  entity_ai: 1n,
  event_spawner: 2n,
  role_mutation: 3n,
...
export function buildPrngStreams(rootSeed: bigint): Record<PrngStream, Pcg32> {
  return {
