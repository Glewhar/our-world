...
import type { DynamicGrid, DynamicChannel } from './grid.js';

export type DecayConfig = Readonly<Record<DynamicChannel, { halfLifeTicks: number }>>;
 ...
 */
export const DEFAULT_DECAY: DecayConfig = Object.freeze({
...
export function decayStep(grid: DynamicGrid, config: DecayConfig = DEFAULT_DECAY): void {
  const factors = {
...
function halfLifeToFactor(halfLifeTicks: number): number {
  if (!Number.isFinite(halfLifeTicks) || halfLifeTicks <= 0) return 1;
