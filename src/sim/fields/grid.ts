...
/** Channels actually backed by this grid (matches the dynamic texture layout). */
export type DynamicChannel = 'fire' | 'ice' | 'infection' | 'pollution';

const DYNAMIC_CHANNELS: readonly DynamicChannel[] = ['fire', 'ice', 'infection', 'pollution'];

const CHANNEL_OFFSET: Readonly<Record<DynamicChannel, 0 | 1 | 2 | 3>> = Object.freeze({
  fire: 0,
  ice: 1,
  infection: 2,
  pollution: 3,
 ...
 */
export function dynamicChannelOffset(attr: AttributeKey): 0 | 1 | 2 | 3 | null {
  switch (attr) {
    case 'fire':
...
/**
 * Per-channel flush payload. `cells` and `values` line up index-for-index;
 * `values` are floats in [0, 1] (i.e. byte / 255) so the main thread doesn't
 * have to know our packing.
 */
...
export class DynamicGrid {
  ...
  private readonly channelDirty: Uint8Array;

  constructor(nside: number, init?: Uint8Array) {
    this.nside = nside;
    this.npix = 12 * nside * nside;
    if (init) {
      if (init.length !== this.npix * 4) {
        throw new Error(
          `DynamicGrid init size mismatch: got ${init.length}, expected ${this.npix * 4}`,
        );
      }
  ...
  setByte(ipix: number, channelOffset: 0 | 1 | 2 | 3, byte: number): void {
    if (ipix < 0 || ipix >= this.npix) return;
  ...
  getByte(ipix: number, channelOffset: 0 | 1 | 2 | 3): number {
    if (ipix < 0 || ipix >= this.npix) return 0;
  ...
  markNonZeroDirty(): void {
    ...
    for (let i = 0; i < this.npix; i++) {
      ...
      this.dirty[i] = 1;
      if (r) anyChannel[0] = true;
      if (g) anyChannel[1] = true;
      if (b) anyChannel[2] = true;
      if (a) anyChannel[3] = true;
    }
    for (let ch = 0; ch < 4; ch++) if (anyChannel[ch]) this.channelDirty[ch] = 1;
  }
   ...
   */
  decayAll(perChannelFactor: Readonly<Record<DynamicChannel, number>>): void {
    for (let ch = 0; ch < 4; ch++) {
      const channelName = DYNAMIC_CHANNELS[ch] as DynamicChannel;
      const factor = perChannelFactor[channelName];
      if (factor >= 1) continue; // identity — skip
      ...
      if (touched) this.channelDirty[ch] = 1;
    }
  }
   ...
   * between events).
   */
  flushDirty(): ChannelDelta[] {
    const deltas: ChannelDelta[] = [];
    if (!this.anyDirty()) return deltas;
    ...
    let count = 0;
    for (let i = 0; i < this.npix; i++) if (this.dirty[i]) count++;

    for (let ch = 0; ch < 4; ch++) {
      if (!this.channelDirty[ch]) continue;
  ...
  private anyDirty(): boolean {
    for (let ch = 0; ch < 4; ch++) if (this.channelDirty[ch]) return true;
    return false;
  }
}
