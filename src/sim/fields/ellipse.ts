...
export type EllipseArgs = {
  /** Peak value at the centre, in [0, 1]. */
  value: number;
  centreLatDeg: number;
  centreLonDeg: number;
  /** Cross-wind half-axis, km. */
  radiusKm: number;
  /** Downwind half-axis, km. */
  stretchKm: number;
  /** Azimuth from north (0 = blowing north, 90 = east). */
  bearingDeg: number;
};

export type EllipseStamp = {
  cells: Uint32Array;
  values: Float32Array;
};
 ...
 */
export function computeEllipseStamp(
  args: EllipseArgs,
  nside: number,
  ...
  for (let i = 0; i < latSteps; i++) {
    ...
    for (let j = 0; j < lonSteps; j++) {
      ...
      if (dRad > halfBoxRad * 1.05) continue; // bbox prefilter
