...
import type { BandPaintArgs } from '../../world/scenarios/types.js';
 ...
 */
function bandIntensityAtLat(latDeg: number, args: BandPaintArgs): number {
  const lo = args.latDegMin;
  const hi = args.latDegMax;
  const soft = args.edgeSoftnessDeg;
  if (latDeg >= lo && latDeg <= hi) return 1;
  if (soft <= 0) return 0;
  if (latDeg < lo) {
    const d = lo - latDeg;
    if (d >= soft) return 0;
 ...
 */
export function computeBandStamp(
  args: BandPaintArgs,
  nside: number,
  ...
  const hasLonWindow =
    args.lonDegMin !== undefined && args.lonDegMax !== undefined;
