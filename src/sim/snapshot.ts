...
import { Pcg32, type PrngStream } from './prng.js';
...
import { DEFAULT_DECAY, type DecayConfig } from './fields/diffusion.js';
...
export type SimSnapshot = {
  ...
  rootSeed: string; // bigint as hex
  tick: number;
  worldTick: number;
  fieldAccumMs: number;
  worldAccumMs: number;
  speed: SimSpeed;
  /** PCG state per stream (hex-encoded {state, inc}). */
  prng: Record<PrngStream, { state: string; inc: string }>;
  /** Base64 of the dynamic grid bytes. */
  fieldBytesB64: string;
  /** Pending events serialised verbatim — `cells` Uint32Arrays land as plain arrays. */
  pendingEvents: SerializedEvent[];
};

type SerializedEvent =
  | { kind: 'plain'; event: WorldEvent }
  | {
      kind: 'cells';
      primitive: WorldEvent['primitive'];
      params: WorldEvent['params'];
      cells: number[];
    };

export function serializeScheduler(s: Scheduler): SimSnapshot {
  const prng: SimSnapshot['prng'] = {
...
export function deserializeScheduler(
  snap: SimSnapshot,
  decay: DecayConfig = DEFAULT_DECAY,
): Scheduler {
  ...
  const s = Object.create(Scheduler.prototype) as Scheduler;
  ...
  Object.defineProperty(s, 'decay' as keyof Scheduler, {
    value: decay,
    enumerable: false,
  });
  s.tick = snap.tick;
...
function serializeEvent(e: WorldEvent): SerializedEvent {
...
function deserializeEvent(s: SerializedEvent): WorldEvent {
  if (s.kind === 'cells') {
    return {
