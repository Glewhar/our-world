...
export class PolygonTexture {
    static async load(attrRef, lookupRef, baseUrl) {
     ...
     */
    classByPolyBytes = [null, null];
    weightByPolyBytes = [null, null];
    tStart01ByPolyBytes = [null, null];
    classByPolyTex = [null, null];
    weightByPolyTex = [null, null];
    tStart01ByPolyTex = [null, null];
    polyTexCache = null;
    constructor(polyBuf, lookup) {
     ...
     * crossfade strength.
     */
    getWeightByPolygonTexture(slot) {
        this.ensureSlotAllocated(slot);
     ...
     */
    getTStart01ByPolygonTexture(slot) {
        this.ensureSlotAllocated(slot);
     ...
     */
    bakeOverrideByPolygon(slot, classByPoly, weightByPoly, tStart01ByPoly) {
        const n = this.lookup.count + 1;
        if (classByPoly.length !== n ||
            weightByPoly.length !== n ||
            tStart01ByPoly.length !== n) {
            throw new Error(`bakeOverrideByPolygon: arrays must be length ${n}; got ` +
    ...
    clearOverrideSlot(slot) {
        if (!this.classByPolyBytes[slot])
            return;
...
function makeR8Texture(bytes, length) {
    const data = bytes;
...
async function fetchAndCheck(url, expectedBytes) {
    ...
    if (buf.byteLength !== expectedBytes) {
        throw new Error(`attribute_polygon size mismatch at ${url}: got ${buf.byteLength}, expected ${expectedBytes}`);
    }
    return buf;
}
function decodeLookup(raw) {
    ...
    const padded = new Array(count + 1);
    padded[0] = {
        id: 0,
        biome: 0,
        realm: 0,
