...
export type HealpixOrdering = 'ring' | 'nested';

const TWO_PI = 2 * Math.PI;
const HALF_PI = Math.PI / 2;

/** Map a unit-sphere point (z, phi) to a HEALPix pixel index. */
export function zPhiToPix(
  nside: number,
  ...
  if (ordering === 'ring') return _angToPixRing(nside, z, tt);
  return _angToPixNested(nside, z, tt);
}

/* ─── internals ─────────────────────────────────────────────────────────── */

function _angToPixRing(nside: number, z: number, tt: number): number {
  const za = Math.abs(z);
  const nl4 = 4 * nside;
...
function _angToPixNested(nside: number, z: number, tt: number): number {
  const za = Math.abs(z);
  let face: number;
  let ix: number;
  let iy: number;
  ...
  if (za <= 2 / 3) {
    ...
    else face = (ifm & 3) + 8; // south face 8..11
