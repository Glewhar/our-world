...
export class DistanceFieldTexture {
    ...
    static async load(url) {
        ...
        if (magic !== DSTF_MAGIC) {
            throw new Error(`distance_field bad magic: 0x${magic.toString(16)} (expected 0x${DSTF_MAGIC.toString(16)})`);
        }
        ...
        if (buf.byteLength !== expectedBytes) {
            throw new Error(`distance_field size mismatch: got ${buf.byteLength}, expected ${expectedBytes} (${width}×${height})`);
        }
