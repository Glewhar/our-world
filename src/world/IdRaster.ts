...
import { type HealpixOrdering, zPhiToPix } from './healpix.js';

const DEG = Math.PI / 180;

export class IdRaster {
  static async load(url: string, nside: number, ordering: HealpixOrdering): Promise<IdRaster> {
    const buffer = await fetchMaybeGz(url);
    const expected = 12 * nside * nside * 4;
    if (buffer.byteLength !== expected) {
      throw new Error(
        `id_raster size mismatch: got ${buffer.byteLength} bytes, expected ${expected} for nside=${nside}`,
      );
    }
  ...
  bodyIndexAtCell(ipix: number): number {
    if (ipix < 0 || ipix >= this.cells.length) return 0;
    return this.cells[ipix] ?? 0;
  }

  bodyIndexAt(latDeg: number, lonDeg: number): number {
