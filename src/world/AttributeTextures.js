...
export class AttributeTextures {
    static async load(refs, baseUrl, nside) {
        ...
        const [staticBuf, climateBuf, dynamicBuf, elevMetersBuf] = await Promise.all([
        ...
        return new AttributeTextures(nside, staticBuf, climateBuf, dynamicBuf, elevMetersBuf);
    }
    nside;
    staticBytes;
    climateBytes;
     ...
     */
    dynamicAttrBytes = new Map();
    dynamicAttrTextures = new Map();
    /**
     ...
     */
    registerDynamicR8Attribute(key) {
        if (this.dynamicAttrBytes.has(key))
            return;
     ...
     */
    getDynamicAttributeTexture(key) {
    ...
    getDynamicAttributeByte(key, ipix) {
        const bytes = this.dynamicAttrBytes.get(key);
        if (!bytes)
            return 0;
        return bytes[ipix] ?? 0;
     ...
     */
    getBiomeOverrideTexture() {
        const cached = this.textures.get('biomeOverride');
        if (cached)
            return cached;
        const w = 4 * this.nside;
        const h = 3 * this.nside;
        const data = this.biomeOverrideClassBytes;
     ...
     */
    getBiomeOverrideStampTexture() {
        const cached = this.textures.get('biomeOverrideStamp');
        if (cached)
            return cached;
        const w = 4 * this.nside;
        const h = 3 * this.nside;
        const data = this.biomeOverrideStampBytes;
