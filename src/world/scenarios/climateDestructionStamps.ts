 ...
 */
export function cellsBelowSeaLevel(
  peakSeaLevelM: number,
  getElevationMetersAtCell: (ipix: number) => number,
  nPix: number,
): Uint32Array {
  if (peakSeaLevelM <= 0) return new Uint32Array(0);
  ...
  for (let i = 0; i < nPix; i++) {
    ...
    if (elev > 0 && elev < peakSeaLevelM) count++;
  }
  const out = new Uint32Array(count);
  let w = 0;
  for (let i = 0; i < nPix; i++) {
    const elev = getElevationMetersAtCell(i);
    if (elev > 0 && elev < peakSeaLevelM) out[w++] = i;
  }
  return out;
}
 ...
 */
export function cellsInProjectedFlipPolygons(
  ...
  if (!flippedAny) return new Uint32Array(0);

  let count = 0;
  for (let i = 0; i < nPix; i++) {
    const polyId = getPolygonOfCell(i);
    if (polyId !== 0 && polyId <= lookup.count && flipSet[polyId]) count++;
  }
  const out = new Uint32Array(count);
  let w = 0;
  for (let i = 0; i < nPix; i++) {
    const polyId = getPolygonOfCell(i);
    if (polyId !== 0 && polyId <= lookup.count && flipSet[polyId]) out[w++] = i;
  }
  return out;
}
