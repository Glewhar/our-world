 ...
 * winter plateau.
 */
export function decaySustained(
  progress01: number,
  holdFrac = 0.85,
  exponent = 2.5,
  rampEnd = 0.02,
  ...
  const p = progress01 < 0 ? 0 : progress01 > 1 ? 1 : progress01;
  if (p < rampEnd) return 0.4 + 0.6 * (p / rampEnd);
  if (p < holdFrac) return 1.0;
 ...
 */
export function infraDecayEnvelope(progress01: number): number {
  const p = progress01 < 0 ? 0 : progress01 > 1 ? 1 : progress01;
 ...
 */
export function rebuildEnvelope(progress01: number): number {
  const p = progress01 < 0 ? 0 : progress01 > 1 ? 1 : progress01;
  return Math.pow(p, 1.5);
}
...
export function climateRisePlateauFall(
  ...
  const p = progress01 < 0 ? 0 : progress01 > 1 ? 1 : progress01;
  if (p < riseFrac) return p / riseFrac;
  if (p > 1 - fallFrac) return (1 - p) / fallFrac;
  return 1;
}
