...
import type { WorldTotals } from '../worldTotals.js';
import type { ClimateContribution, ScenarioCity } from './types.js';
...
export type BiomeAreaChange = {
  readonly fromId: number;
  readonly toId: number;
  /** Land fraction of the planet affected, [0, 1]. */
  readonly areaFraction: number;
  /** Signed: quality(to) − quality(from). Positive = upgrade. */
  readonly qualityDelta: number;
};

export type ScenarioImpactBudget = {
  populationAtRisk: number;
  citiesAtRisk: number;
  /** Array, not Map — order is stable for HUD top-3 readout. */
  biomeChanges: BiomeAreaChange[];
  /** Σ areaFraction × qualityDelta. Signed roll-up the HUD reads. */
 ...
 */
function addBiomeChange(
  budget: ScenarioImpactBudget,
  fromId: number,
  toId: number,
  areaFraction: number,
): void {
...
/** Geometry of one nuclear-strike kill ellipse. */
export type StrikeEllipse = {
  centreLatDeg: number;
  centreLonDeg: number;
  /** Cross-wind half-axis in km (already scaled by killRadiusMultiplier). */
  radiusKm: number;
  /** Downwind half-axis in km. */
  stretchKm: number;
  /** Azimuth from north in degrees. */
  bearingDeg: number;
};

export type ImpactBudgetDeps = {
  cities: readonly ScenarioCity[];
  polygonLookup: PolygonLookup;
  totals: WorldTotals;
  /** Equirect raster lookup of polygon id at (lat, lon). */
  getPolygonIdAt(latDeg: number, lonDeg: number): number;
  /**
   * Combined effective climate frame seen by the planet at the moment
   * this budget is computed. Climate handlers run `tallyProjectionBiome`
   * against this delta so HUD impact tracks the GPU biome paint exactly
   * — when two climate scenarios cancel, the projection produces small
   * weights and the HUD bar barely moves.
   */
  combinedClimate: ClimateContribution;
};

export function zeroBudget(): ScenarioImpactBudget {
  return {
    populationAtRisk: 0,
    citiesAtRisk: 0,
    biomeChanges: [],
 ...
 */
function strikeBbox(e: StrikeEllipse): {
  latMin: number;
  latMax: number;
  lonMin: number;
  lonMax: number;
 ...
 */
export function tallyStrikeCities(
  e: StrikeEllipse,
  cities: readonly ScenarioCity[],
  budget: ScenarioImpactBudget,
  cityHitMask: Uint8Array | null = null,
): void {
  const bb = strikeBbox(e);
  const wrapLon = bb.lonMin < -180 || bb.lonMax > 180;
  for (let i = 0; i < cities.length; i++) {
    if (cityHitMask && cityHitMask[i]) continue;
    const c = cities[i]!;
    if (c.latDeg < bb.latMin || c.latDeg > bb.latMax) continue;
    if (!wrapLon && (c.lonDeg < bb.lonMin || c.lonDeg > bb.lonMax)) continue;
    if (!pointInEllipse(c.latDeg, c.lonDeg, e)) continue;
    if (cityHitMask) cityHitMask[i] = 1;
 ...
 */
export function tallyStrikeBiome(
  e: StrikeEllipse,
  deps: ImpactBudgetDeps,
  budget: ScenarioImpactBudget,
  polyHitMask: Uint8Array | null = null,
): void {
  const bb = strikeBbox(e);
  const lookup = deps.polygonLookup;
  const landAreaProxy = deps.totals.landAreaProxy;
  if (landAreaProxy <= 0) return;
  for (let i = 1; i <= lookup.count; i++) {
    if (polyHitMask && polyHitMask[i]) continue;
    if (lookup.latMax[i]! < bb.latMin) continue;
    if (lookup.latMin[i]! > bb.latMax) continue;
    if (lookup.lonMax[i]! < bb.lonMin) continue;
    if (lookup.lonMin[i]! > bb.lonMax) continue;
    if (!pointInEllipse(lookup.latC[i]!, lookup.lonC[i]!, e)) continue;
    const cls = lookup.biome[i]! & 0xff;
    if (cls === 0) continue;
    if (polyHitMask) polyHitMask[i] = 1;
 ...
 */
export function tallyProjectionBiome(
  delta: ClimateDelta,
  deps: ImpactBudgetDeps,
  budget: ScenarioImpactBudget,
): void {
  ...
  for (let i = 1; i <= lookup.count; i++) {
    ...
    if (proj.weight <= 0 || proj.toId === proj.fromId) continue;
    const area = polygonBboxAreaKm2(lookup, i);
  ...
  if (iceFlipAny) {
    ...
    for (let i = 0; i < cities.length; i++) {
      ...
      if (polyId <= 0 || polyId > lookup.count) continue;
      if (!iceFlipPolyMask[polyId]) continue;
      budget.citiesAtRisk += 1;
 ...
 * / tundra retain it.
 */
export function tallyStrikeBiomeBlt(
  e: StrikeEllipse,
  deps: ImpactBudgetDeps,
  budget: ScenarioImpactBudget,
  polyHitMask: Uint8Array | null = null,
