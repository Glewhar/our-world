...
import { BIOME, BIOME_LOOKUP, type BiomeEntry } from '../biomes/BiomeLookup.js';
import {
  MAG_TEMP_REF,
  MAG_PRECIP_REF,
  MIN_DELTA_TO_FLIP,
  WEIGHT_NORMALISER,
  VULNERABILITY_ORDER,
} from './biomeProjection.config.js';
...
export type ProjectedTransition = {
  fromId: number;
  toId: number;
  /** Stamp weight in [0, 1] — how completely the polygon converts. */
  weight: number;
  /** Onset offset in [0, 1] — vulnerability ordering for the LAND shader. */
  tStart01: number;
};
 ...
 */
export function deltaMagnitude(delta: ClimateDelta): number {
 ...
 */
function scoreNiche(entry: BiomeEntry, effTempC: number, effPrecipMm: number): number {
 ...
 * score by default.
 */
export function projectBiome(baselineId: number, delta: ClimateDelta): ProjectedTransition {
  const noFlip: ProjectedTransition = {
    fromId: baselineId,
    toId: baselineId,
    weight: 0,
    tStart01: 0,
  ...
  if (!baseline || baseline.id === 0) return noFlip;

  const mag = deltaMagnitude(delta);
  if (mag < MIN_DELTA_TO_FLIP) return noFlip;
...
/**
 * Per-polygon textures consumed by `BiomeOverrideEquirect.bakePolygonOverride`.
 * Length is `polygonLookup.count + 1`; slot 0 is the no-data sentinel.
 */
export type ProjectionPolygonOutput = {
  classByPoly: Uint8Array;
  weightByPoly: Uint8Array;
  tStart01ByPoly: Uint8Array;
  /** Number of polygons that received a non-zero flip — for telemetry / logs. */
  assignedCount: number;
};

export type ProjectionPolygonInput = {
  /** Polygon column-store. Reads `biome` (baseline class) per row. */
  polygonBiome: Int8Array;
  /** Polygon count (matches `polygonBiome.length - 1`). */
  count: number;
  delta: ClimateDelta;
  /**
   * Weight multiplier in [0, 1] — used to split the projection between
   * the two climate slots when both are active. With one active slot,
   * pass `1`. With two, pass `0.5` so the GPU sum stays consistent.
   */
  weightScale: number;
};
 ...
 */
export function buildProjectionPolygonTextures(
  input: ProjectionPolygonInput,
): ProjectionPolygonOutput {
  ...
  let assigned = 0;
  if (scale <= 0) return { classByPoly, weightByPoly, tStart01ByPoly, assignedCount: 0 };
