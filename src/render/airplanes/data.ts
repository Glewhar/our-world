...
export type RoutesFile = {
  version: number;
  routes: { a: number; b: number; w: number }[];
};

export type AirplaneData = {
  /** Per-airport (lat, lon) packed as [lat0, lon0, lat1, lon1, ...]. */
  airportLatLons: Float32Array;
  /** Per-airport traffic weight (sum of incident route weights). */
  airportTraffic: Float32Array;
  /** Per-airport IATA code (kept for picking + debug). */
  airportIatas: string[];

  /** Per-route src airport index. */
  routeSrc: Uint32Array;
  /** Per-route dst airport index. */
  routeDst: Uint32Array;
  /** Per-route normalised weight in [0,1]. */
  routeWeight: Float32Array;
  /** Per-route great-circle distance in km (cached for plane spawn timing). */
  routeDistanceKm: Float32Array;
  /** Per-route midpoint longitude in radians (for day/night modulation). */
  routeMidpointLon: Float32Array;
  /** Per-route midpoint latitude in radians. */
  routeMidpointLat: Float32Array;
};
...
function greatCircleKm(latA: number, lonA: number, latB: number, lonB: number): number {
  const φ1 = latA * DEG;
  const φ2 = latB * DEG;
...
export async function loadAirplaneData(): Promise<AirplaneData> {
  const [airportsResp, routesResp] = await Promise.all([
    fetch(AIRPORTS_URL),
    fetch(ROUTES_URL),
  ]);
  if (!airportsResp.ok) throw new Error(`airports.json: ${airportsResp.status}`);
  if (!routesResp.ok) throw new Error(`routes.json: ${routesResp.status}`);
  const airportsFile = (await airportsResp.json()) as AirportsFile;
  const routesFile = (await routesResp.json()) as RoutesFile;

  const aN = airportsFile.airports.length;
  const airportLatLons = new Float32Array(aN * 2);
  const airportTraffic = new Float32Array(aN);
  const airportIatas: string[] = new Array(aN);
  for (let i = 0; i < aN; i++) {
  ...
  console.info(
    `[airplanes] loaded ${aN} airports, ${rN} routes ` +
