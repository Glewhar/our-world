...
export class AirplaneSystem {
    ...
    setLayerActive(layer, on) {
        if (layer === 'airports')
            this.airports.setActive(on);
        else if (layer === 'scaffold')
            this.scaffold.setActive(on);
        else if (layer === 'trails')
            this.trails.setActive(on);
        else if (layer === 'heads')
            this.heads.setActive(on);
    }
    setViewport(w, h) {
     ...
     */
    prefillToSteadyState() {
        if (this.targetInFlight <= 0)
            return;
    ...
    repackInstances() {
        const trailSrcDst = this.trails.aSrcDstAttr.array;
        const trailTMin = this.trails.aTMinAttr.array;
        const trailTMax = this.trails.aTMaxAttr.array;
        const trailAlpha = this.trails.aAlphaAttr.array;
        const headSrcDst = this.heads.aSrcDstAttr.array;
        const headT = this.heads.aTAttr.array;
