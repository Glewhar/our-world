...
export function createSceneGraph() {
    ...
    const paletteColors = Array.from({ length: 20 }, () => new THREE.Color());
    // Last css string per palette slot — skip `.set(css)` when unchanged so we
    // don't re-parse a hex string 20×/frame.
    const paletteCssCache = Array.from({ length: 20 }, () => null);
    ...
    let stashedClimateFrame = { tempC: 0, seaLevelM: 0 };
    let stashedOverrideTex = null;
    let stashedOverrideStampTex = null;
    const stashedClimateEnvelopes = [0, 0];
    let stashedSeafloorFrame = {
        paletteA: [
            { r: 0, g: 0, b: 0 },
            { r: 0, g: 0, b: 0 },
            { r: 0, g: 0, b: 0 },
    ...
    function applyMaterials(debug) {
        ...
        if (cloudsPass) {
            ...
            cloudsPass.setSootFrame(stashedCloudFrame.sootGlobal, stashedCloudFrame.sootRegionalWeight, stashedCloudFrame.sootSunTint, stashedCloudFrame.sootAmbientTint);
        }
        ...
        if (highways) {
            ...
            u.uNightColor.value.set(hz.nightColor);
            u.uDayCasingColor.value.set(hz.dayCasingColor);
            u.uDayFillColor.value.set(hz.dayFillColor);
        }
        if (cities) {
            ...
            cu.uNightFillColor.value.set(ct.nightFillColor);
            cu.uNightOutlineColor.value.set(ct.nightOutlineColor);
            cu.uDayNeutralColor.value.set(ct.dayNeutralColor);
        }
        ...
        const finalSeaLevelM = debug.materials.ocean.seaLevelOffsetM + stashedClimateFrame.seaLevelM;
    ...
    function detonateAt(direction, elevationM, wind, sizeKm) {
