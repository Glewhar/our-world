...
void main() {
  ...
  if (uHasPolyFlipMask == 1 && uDestructionIntensity > 0.0) {
    float lonC = atan(sphereDir.y, sphereDir.x);
    float latC = asin(clamp(sphereDir.z, -1.0, 1.0));
    vec2 uv = vec2(lonC * 0.15915494 + 0.5, 0.5 - latC * 0.31830989);
    ivec2 ppx = ivec2(
...
export type HighwaysUniforms = {
  ...
  uDayCasingColor: { value: THREE.Color };
  uDayFillColor: { value: THREE.Color };
 ...
 */
function hashLatLonToUnit(latDeg: number, lonDeg: number): number {
...
export class HighwaysLayer {
  ...
  constructor(world: WorldRuntime, roads: readonly RoadRecord[]) {
    ...
    const coastFade: CoastFadeStrategy = distFieldTex
      ? {
          kind: 'distance-field',
          ...
          data: distFieldTex.image.data as unknown as Uint16Array,
