...
export type CoastFadeStrategy =
  | { kind: 'ocean-count-8' }
  | {
      kind: 'distance-field';
      /** RG half-float body; coast-km in channel R, biome-km in G. */
      data: Uint16Array;
      width: number;
      height: number;
    };

export type LiftBakeContext = {
  world: WorldRuntime;
  nside: number;
  ordering: HealpixOrdering;
  coastFade: CoastFadeStrategy;
};
 ...
 * read channel R.
 */
function sampleDistFieldCoastKm(
  dirX: number,
  dirY: number,
  dirZ: number,
  df: { data: Uint16Array; width: number; height: number },
): number {
 ...
 */
export function bakeLiftMeters(
  dirX: number,
  dirY: number,
  dirZ: number,
  ctx: LiftBakeContext,
  ...
  for (let i = 0; i < 9; i++) {
    ...
    if (i > 0 && ctx.world.getBodyIndexAtCell(ipix) === 0) oceanCount++;
  }
  const elev = elevSum / 9;

  let coastFade: number;
