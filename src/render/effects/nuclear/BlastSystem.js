...
export class BlastSystem {
    ...
    constructor(camera, profile) {
        ...
        for (let i = 0; i < MAX_CONCURRENT_BLASTS; i++) {
            const particles = new Array(maxN);
    ...
    update(deltaSec) {
        ...
        for (let s = 0; s < this._slots.length; s++) {
            ...
            if (slot.worldOrigin.dot(this._camera.position) < FAR_SIDE_CULL_DOT)
                continue;
    ...
    // ── Internals ──────────────────────────────────────────────────────────
    _initSlot(s, direction, radius, wind, sizeKm) {
        const slot = this._slots[s];
    ...
    _initParticle(p, cfg) {
        ...
        const xz_z = dirZ * k;
        const radiusPerc = cfg.maxRadius === cfg.minRadius
            ? 1
    ...
    _sortSlot(slot) {
        ...
        if (scale > 1e-9)
            this._tmpCam.multiplyScalar(1 / scale);
    ...
    _writeSlot(s, slot) {
        ...
        if (posRangeCount > 0)
            this._posAttr.addUpdateRange(posRangeStart, posRangeCount);
        this._sizeAttr.addUpdateRange(sizeRangeStart, sizeRangeCount);
        if (colRangeCount > 0)
            this._colAttr.addUpdateRange(colRangeStart, colRangeCount);
