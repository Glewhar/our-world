...
export class UrbanDetailLayer {
    ...
    static ENGAGE_DISTANCE_KM = 800;
    /** km — once the closest city is farther than this, drop the build. */
    static DISENGAGE_DISTANCE_KM = 1100;
    constructor(_world, urbanAreas) {
     ...
     */
    update(cameraPos) {
        if (!this.layerVisible || this.polyMeta.length === 0)
            return;
    ...
    buildCityMesh(meta) {
        ...
        for (let r = 0; r < MASK_SIZE; r++) {
            if (r % streetCellStrideY !== 0)
                continue;
            for (let c = 0; c < MASK_SIZE; c++) {
                if (mask[r * MASK_SIZE + c])
                    isStreet[r * MASK_SIZE + c] = 1;
            }
        }
        for (let c = 0; c < MASK_SIZE; c++) {
            if (c % streetCellStrideX !== 0)
                continue;
            for (let r = 0; r < MASK_SIZE; r++) {
                if (mask[r * MASK_SIZE + c])
                    isStreet[r * MASK_SIZE + c] = 1;
        ...
        const localXY = []; // per-building (x_km, y_km)
        const footprint = []; // per-building (width_m, depth_m)
        const heights = []; // per-building height_m
        const rotations = []; // per-building rotation radians (axis-aligned ± jitter)
        let buildingCount = 0;
        outer: for (let r = 0; r < MASK_SIZE; r++) {
            for (let c = 0; c < MASK_SIZE; c++) {
                const idxCell = r * MASK_SIZE + c;
                if (!mask[idxCell] || isStreet[idxCell])
                    continue;
                if (buildingCount >= MAX_BUILDINGS_PER_CITY)
                    break outer;
        ...
        if (this.buildingGeometry.index)
            bldGeom.setIndex(this.buildingGeometry.index);
        bldGeom.setAttribute('aLocalXY', new THREE.InstancedBufferAttribute(new Float32Array(localXY), 2));
        bldGeom.setAttribute('aSize', new THREE.InstancedBufferAttribute(new Float32Array(flatten3(footprint, heights)), 3));
        ...
        const { positions: strPositions, uvs: strUvs } = buildStreetGeometry(isStreet, mask, meta, MASK_SIZE);
...
function flatten3(footprint, heights) {
    const out = new Array(heights.length * 3);
...
function rasteriseInsideMask(meta, size) {
 ...
 */
function buildStreetGeometry(isStreet, mask, meta, size) {
    ...
    let streetCells = 0;
    for (let i = 0; i < isStreet.length; i++)
        if (isStreet[i] && mask[i])
            streetCells++;
    ...
    for (let r = 0; r < size; r++) {
        ...
        for (let c = 0; c < size; c++) {
            const idx = r * size + c;
            if (!isStreet[idx] || !mask[idx])
                continue;
            const x0 = (-meta.halfExtentKm.x) + c * cellW;
            const x1 = x0 + cellW;
            // Triangle 1
            positions[writePos++] = x0;
            positions[writePos++] = y0;
            positions[writePos++] = x1;
            positions[writePos++] = y0;
            positions[writePos++] = x1;
            positions[writePos++] = y1;
            // Triangle 2
            positions[writePos++] = x0;
            positions[writePos++] = y0;
            positions[writePos++] = x1;
