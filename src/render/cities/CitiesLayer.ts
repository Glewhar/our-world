...
export type CitiesUniforms = {
  ...
  /** Shared half-quad size in unit-sphere units (max half-extent across all polygons). */
  uHalfQuadSizeUnit: { value: number };
  uHalfQuadSizeKm: { value: number };

  /** Polygon-vertex atlas (RG32F, vertex positions in tangent-km). */
  uPolyAtlas: { value: THREE.DataTexture | null };
  uPolyAtlasWidth: { value: number };
...
export class CitiesLayer {
  ...
  private readonly atlasTexture: THREE.DataTexture | null;
  ...
  constructor(world: WorldRuntime, urbanAreas: readonly UrbanAreaRecord[]) {
    ...
    let atlasMeta: ReturnType<typeof buildPolygonAtlas>['meta'] = [];
