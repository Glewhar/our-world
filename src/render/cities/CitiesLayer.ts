...
export type CitiesUniforms = {
  ...
  uNightFillColor: { value: THREE.Color };
  uNightOutlineColor: { value: THREE.Color };
  uDayNeutralColor: { value: THREE.Color };
...
function buildBucketGeometry(
  ...
  // First pass — triangulate each tier of each city. One Triangulated
  // entry per tier; the per-vertex `density` is propagated to every
  // vertex of that tier so the fragment shader reads it directly. All
  // tiers of a city share the same `aPatternSeed` (= record.id) so the
  // block-spray pattern stays coherent across tier boundaries.
  type Triangulated = {
    record: UrbanAreaRecord;
    density: number;
    /** Unit-sphere (x,y,z) per tier vertex. */
    positions: Float32Array;
    /** Tangent-frame (x_km, y_km) per tier vertex. */
    localKm: Float32Array;
    halfExtentKm: { x: number; y: number };
    /** Flat triangle indices into the tier's own vertex array. */
    indices: Uint32Array;
  };
