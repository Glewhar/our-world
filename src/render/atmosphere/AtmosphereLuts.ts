...
function createMultiScatteringMaterial(
...
function createSkyViewMaterial(
  transmittanceTex: THREE.Texture,
...
export class AtmosphereLuts {
  ...
  constructor(
    ...
    const irradiance =
      opts.solarIrradiance?.clone() ?? new THREE.Vector3(1.474, 1.8504, 1.91198);
  ...
  setSolarIrradiance(r: number, g: number, b: number): void {
    (this.multiScatteringMat.uniforms.uSolarIrradiance!.value as THREE.Vector3).set(r, g, b);
    (this.skyViewMat.uniforms.uSolarIrradiance!.value as THREE.Vector3).set(r, g, b);
