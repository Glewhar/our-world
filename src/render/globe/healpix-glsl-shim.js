...
function healpixXyToMorton(ix, iy) {
 ...
 */
function healpixFloorDiv(a, b) {
    return Math.floor(a / b);
}
function healpixAngToPixRing(nside, z, tt) {
    const za = Math.abs(z);
    const nl4 = 4 * nside;
    const fns = nside;
...
function healpixAngToPixNested(nside, z, tt) {
    const za = Math.abs(z);
    let face;
    let ix;
    let iy;
...
export function zPhiToPixGlslShim(nside, ordering, z, phi) {
    ...
    if (ordering === 'ring')
        return healpixAngToPixRing(nside, z, tt);
