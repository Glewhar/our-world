...
function clampByteUnit(v: number): number {
...
export class BiomeOverrideEquirect {
  ...
  private cachedClassByPolyVer = -1;
  private cachedWeightByPolyVer = -1;
  private cachedSigma = -1;
  private pendingSigma = BLUR_SIGMA_PX;

  private readonly classByPolyTex: THREE.DataTexture;
  private readonly weightByPolyTex: THREE.DataTexture;

  constructor(world: WorldRuntime, slot: 0 | 1 = 0) {
    const polyTex = world.getPolygonTexture();
    const polyLookup: PolygonLookup | null = world.getPolygonLookup();
    if (!polyTex || !polyLookup) {
      throw new Error(
        'BiomeOverrideEquirect: polygon artifacts missing — `attribute_polygon` and `polygon_lookup` are now required',
      );
    }
    ...
    this.paletteTexture = new THREE.DataTexture(
      new Uint8Array(PALETTE_SIZE * 4) as Uint8Array<ArrayBuffer>,
      PALETTE_SIZE,
      1,
      THREE.RGBAFormat,
  ...
  rebuildIfDirty(
    ...
    if (classVer !== this.cachedClassByPolyVer) this.dirty = true;
    if (weightVer !== this.cachedWeightByPolyVer) this.dirty = true;
